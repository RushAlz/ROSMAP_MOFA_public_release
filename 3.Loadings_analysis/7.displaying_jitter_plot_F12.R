...
all_weights=get_weights(mofa,factors = 12,scale = TRUE, as.data.frame = TRUE)
...
##########################################################
# DISPLAY JITTER PLOT WITH FEATURE WEIGHTS FOR FACTOR 12 #
##########################################################
library(readxl)
library(patchwork)
library(ggplot2)
...
jitter_ploting_weights <- function(df,categories,filtered_val){
  ...
  # Adding legend
  acetil_plot <- acetil_plot + guides(color = guide_legend(override.aes = list(size = 7)))
...
df_weights_F12 <- cleared_df

# A custom vector of genes related to AD
load("C:/Users/User/Documents/Drive_PC/Lucas/GitHub/ROSMAP-MOFA/MOFA_real/Raw_data/weights/F12_gene_vec")
...
val_filter = 0.5
df_filtered <- filter(df_weights_F12, value > val_filter)

##################################################### Chosen pathways #############################################################################

# Retrieving the genes from the pathways lists

energy=ALL_DATA_BASE[grep("GOBP_GENERATION_OF_PRECURSOR_METABOLITES_AND", names(ALL_DATA_BASE))]
energy=unlist(energy)
energy = unique(energy)
energy=intersect(df_filtered$feature,energy)

apoptosis = ALL_DATA_BASE[grep("GOBP_APOPTOTIC_SIGNALING_PATHWAY", names(ALL_DATA_BASE))]
apoptosis=unlist(apoptosis)
apoptosis = unique(apoptosis)
apoptosis=intersect(df_filtered$feature,apoptosis)

myelin <- MODULES_LIST[grep("m3_oligo_myelination",names(MODULES_LIST))]
myelin = unlist(myelin)
myelin = unique(myelin)
myelin = intersect(df_filtered$feature,myelin)

# Concatenating genes in a vector
combined_genes  = c(energy,apoptosis,myelin,g_was)
...
df_weights_F12$real_values <- ifelse(df_weights_F12$sign == '-', df_weights_F12$value*-1, df_weights_F12$value*1)
df_weights_F12$category <- 'genes'

# Filtering by genes present in 'combined_genes' and by 'val_filter'
df_weights_F12x <- filter(df_weights_F12, feature %in% combined_genes)
df_weights_F12x <- filter(df_weights_F12x, value > val_filter)

# Updating the 'category' column values according to each pathway
df_weights_F12x <- df_weights_F12x %>%
  mutate(
    category = case_when(
      feature %in% g_was ~ 'AD GWAS',
      feature %in% genes_in_more_than_one ~ 'Multiple pathways',
      feature %in% energy ~ 'Generation of precursor\nmetabolites and energy',
      feature %in% apoptosis ~ 'Apoptotic signaling pathway',
      feature %in% myelin ~ 'Prot-M3 (oligo myelination)',
      TRUE ~ 'genes'
    )
  )

# Filtering by genes not present in 'combined_genes' and by 'val_filter'
df_weights_F12 <- filter(df_weights_F12, !(feature %in% combined_genes) | value <= val_filter)

#' Recombining 'df_weights_F12x' and 'df_weights_F12' to restore the original data frame structure, 
#' but now with different values on the column 'category'
df_weights_F12 <- bind_rows(df_weights_F12, df_weights_F12x)

# Rearranging order
df_weights_F12$category <- factor(df_weights_F12$category, levels = c(
  'AD GWAS',
  'Multiple pathways',
  'Apoptotic signaling pathway',
  'Prot-M3 (oligo myelination)',
  'Generation of precursor\nmetabolites and energy',
  "genes")
)

# Display plot
jitter_plot_F12 <- jitter_ploting_weights(df_weights_F12,filtered_val = val_filter, 
                                          categories = c('AD GWAS' = 'black',
                                                         'Apoptotic signaling pathway' = "magenta",
                                                         'Prot-M3 (oligo myelination)' = "blue",
                                                         'Generation of precursor\nmetabolites and energy' = "#CC0000FF",
                                                         'Multiple pathways' = 'darkgreen'
                                          ))

ggsave("C:/Users/User/Documents/Drive_PC/Lucas/GitHub/ROSMAP-MOFA/MOFA_real/Figuras_MOFA/pre_inkscape/weight/jitter/new/new_jitter_F12_v6.pdf",
       plot = jitter_plot_F12 , width = 500, height = 255, units = 'mm')
