...
all_weights=get_weights(mofa,factors = 8,scale = TRUE, as.data.frame = TRUE)
...
jitter_ploting_weights <- function(df,categories,filtered_val){
  ...
  AC_plot <- AC_plot + guides(color = guide_legend(override.aes = list(size = 7)))
...
df_weights_F8 <- cleared_df

# A custom vector of genes related to AD
load("C:/Users/User/Documents/Drive_PC/Lucas/GitHub/ROSMAP-MOFA/MOFA_real/Raw_data/weights/F8_gene_vec")
...
repeated_genes <- c("BAG3","HSP90AA1", "HSP90AB1", "HSPA1A")

# Threshold for filtering feature weights by absolute value
val_filter = 0.5
df_filtered <- filter(df_weights_F8, value > val_filter)
length(unique(df_filtered$feature))
##################################################### Chosen pathways ##############################################################################

# Retrieving the genes from the pathways lists

cell_signaling <- ALL_DATA_BASE[grep("GOBP_CELL_CELL_SIGNALING$",names(ALL_DATA_BASE))]
cell_signaling <- unlist(cell_signaling)
cell_signaling <- unique(cell_signaling)
cell_signaling <- intersect(df_filtered$feature,cell_signaling)

prot_folding =ALL_DATA_BASE[grep("GOBP_PROTEIN_FOLDING$",names(ALL_DATA_BASE))]
prot_folding=unlist(prot_folding)
prot_folding = unique(prot_folding)
prot_folding=intersect(df_filtered$feature, prot_folding)
prot_folding <- c(prot_folding, "DERL3")

transporters=ALL_DATA_BASE[grep("GOMF_TRANSPORTER_ACTIVITY$", names(ALL_DATA_BASE))]
transporters=unlist(transporters)
transporters=unname(transporters)
transporters=intersect(df_filtered$feature, transporters)

non_coding <- c('TMEM51-AS1', 'FAM225A', 'FAM225B', 'MELTF-AS1', 'FOXN3-AS1', 'LINC01736', 'LINC01532', 'LINC02192', 'PROSER2-AS1', 'BAALC-AS1', 'ZFHX4-AS1')

# Concatenating genes in a vector
combined_genes  = c(cell_signaling, prot_folding, transporters ,g_was, non_coding)
...
df_weights_F8$real_values <- ifelse(df_weights_F8$sign == '-', df_weights_F8$value*-1, df_weights_F8$value*1)
df_weights_F8$category <- 'genes'

# Filtering by genes present in 'combined_genes' and by 'val_filter'
df_weights_F8x <- filter(df_weights_F8, feature %in% combined_genes)
df_weights_F8x <- filter(df_weights_F8x, value > val_filter)

# Updating the 'category' column values according to each pathway
df_weights_F8x <- df_weights_F8x %>%
  mutate(
    category = case_when(
      feature %in% g_was ~ 'AD GWAS',
      feature %in% genes_in_more_than_one ~ 'Multiple pathways',
      feature %in% prot_folding ~'Protein folding',
      feature %in% cell_signaling ~ 'Cell-cell signaling',
      feature %in% transporters ~ 'Transporters',
      feature %in% non_coding ~ 'lnc RNA',
      TRUE ~ 'genes'
    )
  )

# Filtering by genes not present in 'combined_genes' and by 'val_filter'
df_weights_F8 <- filter(df_weights_F8, !(feature %in% combined_genes) | value <= val_filter)

#' Recombining 'df_weights_F8x' and 'df_weights_F8' to restore the original data frame structure, 
#' but now with different values on the column 'category'
final_F8_df <- bind_rows(df_weights_F8, df_weights_F8x)

# Rearranging order
final_F8_df$category <- factor(final_F8_df$category, levels = c(
  'AD GWAS',
  'Multiple pathways',
  'Cell-cell signaling',
  "Protein folding",
...
# Display plot
jitter_plot_F8 <- jitter_ploting_weights(final_F8_df,filtered_val = val_filter, 
                                         categories = c(
                                           'AD GWAS' = 'black',
                                           'Multiple pathways' = 'darkgreen',
                                           'Cell-cell signaling' = "#631879FF",
                                           'Protein folding' = "#99991EFF",
                                           'Transporters' = 'blue',
                                           'lnc RNA' = 'orange'
                                           )
                                         )

ggsave("C:/Users/User/Documents/Drive_PC/Lucas/GitHub/ROSMAP-MOFA/MOFA_real/Figuras_MOFA/pre_inkscape/weight/jitter/new/new_jitter_F8_v6.pdf",
       plot = jitter_plot_F8 , width = 500, height = 255, units = 'mm')
