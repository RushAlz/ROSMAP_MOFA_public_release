...
all_weights=get_weights(mofa,factors = 1,scale = TRUE, as.data.frame = TRUE)
...
#########################################################
# DISPLAY JITTER PLOT WITH FEATURE WEIGHTS FOR FACTOR 1 #
#########################################################
library(readxl)
library(patchwork)
library(ggplot2)
...
df_weights_F1 <- cleared_df

# A custom vector of genes related to AD
load("C:/Users/User/Documents/Drive_PC/Lucas/GitHub/ROSMAP-MOFA/MOFA_real/Raw_data/weights/F1_gene_vec")
...
val_filter = 0.5
df_filtered <- filter(df_weights_F1, value > val_filter)

##################################################### Chosen pathways ##############################################################################

# Retrieving the genes from the pathways lists

chap_prot_folding <- ALL_DATA_BASE[grep("CHAPERONE_MEDIATED_PROTEIN_FOLDING", names(ALL_DATA_BASE))]
chap_prot_folding <- unlist(chap_prot_folding)
chap_prot_folding <- unique(chap_prot_folding)
chap_prot_folding <- intersect(df_filtered$feature,chap_prot_folding)

transporter <- ALL_DATA_BASE[grep('GOMF_TRANSPORTER_ACTIVITY$', names(ALL_DATA_BASE))]
transporter <- unlist(transporter)
transporter <- unique(transporter)
transporter <- intersect(df_filtered$feature,transporter)

snc <- ALL_DATA_BASE[grep('CENTRAL_NERVOUS_SYSTEM_DEVELOPMENT', names(ALL_DATA_BASE))]
snc <- unlist(snc)
snc <- unique(snc)
snc <- intersect(df_filtered$feature,snc)

# Concatenating genes in a vector
combined_genes  = c(chap_prot_folding,transporter,snc,g_was)
...
df_weights_F1$real_values <- ifelse(df_weights_F1$sign == '-', df_weights_F1$value*-1, df_weights_F1$value*1)
df_weights_F1$category <- 'genes'

# Filtering by genes present in 'combined_genes' and by 'val_filter' 
df_weights_F1x <- filter(df_weights_F1, feature %in% combined_genes)
df_weights_F1x <- filter(df_weights_F1x, value > val_filter)

# Updating the 'category' column values according to each pathway
df_weights_F1x <- df_weights_F1x %>%
  mutate(
    category = case_when(
      feature %in% g_was ~ 'AD GWAS',
      feature %in% genes_in_more_than_one ~ 'Multiple pathways',
      feature %in% transporter ~ 'Transporters',
      feature %in% chap_prot_folding ~ 'Chaperone mediated\nprotein folding',
      feature %in% snc ~ 'SNC development',
      TRUE ~ 'genes'
    )
  )

# Filtering by genes not present in 'combined_genes' and by 'val_filter'
df_weights_F1 <- filter(df_weights_F1, !(feature %in% combined_genes) | value <= val_filter)

#' Recombining 'df_weights_F1x' and 'df_weights_F1' to restore the original data frame structure, 
#' but now with different values on the column 'category'
df_weights_F1 <- bind_rows(df_weights_F1, df_weights_F1x)

# Rearranging order
df_weights_F1$category <- factor(df_weights_F1$category, levels = c(
  'AD GWAS',
  'Multiple pathways',
  'Transporters',
  'SNC development',
...
# Display plot
jitter_plot_F1 <- jitter_ploting_weights(df_weights_F1,filtered_val = val_filter, 
                                         categories = c('AD GWAS' = 'black',
                                                        'SNC development' = "magenta",
                                                        'Transporters' = "blue",
                                                        'Chaperone mediated\nprotein folding' = "#631879FF",
                                                        'Multiple pathways' = 'darkgreen'
                                         )
)

# ggsave("C:/Users/User/Documents/Drive_PC/Lucas/GitHub/ROSMAP-MOFA/MOFA_real/Figuras_MOFA/pre_inkscape/weight/jitter/new/new_jitter_F1_v6.pdf",
#        plot = jitter_plot_F1 , width = 500, height = 255, units = 'mm')
