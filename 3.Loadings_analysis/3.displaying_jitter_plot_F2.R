...
all_weights=get_weights(mofa,factors = 2,scale = TRUE, as.data.frame = TRUE)
...
#########################################################
# DISPLAY JITTER PLOT WITH FEATURE WEIGHTS FOR FACTOR 2 #
#########################################################
library(readxl)
library(patchwork)
library(ggplot2)
...
df_weights_F2 <- cleared_df

# A custom vector of genes related to AD
load("C:/Users/User/Documents/Drive_PC/Lucas/GitHub/ROSMAP-MOFA/MOFA_real/Raw_data/weights/F2_gene_vec")
...
val_filter = 0.5
df_filtered <- filter(df_weights_F2, value > val_filter)

##################################################### Chosen pathways ##############################################################################

# Retrieving the genes from the pathways lists

immune <- ALL_DATA_BASE[grep("GOBP_IMMUNE_RESPONSE$", names(ALL_DATA_BASE))]
immune <- unlist(immune)
immune <- unique(immune)
immune <- intersect(df_filtered$feature,immune)

vasc <- ALL_DATA_BASE[grep("CIRCULATORY_SYSTEM_DEVELOPMENT$", names(ALL_DATA_BASE))]
vasc <- unlist(vasc)
vasc <- unique(vasc)
vasc <- intersect(df_filtered$feature,vasc)

# Concatenating genes in a vector
combined_genes  = c(immune,vasc,g_was)
...
df_weights_F2$real_values <- ifelse(df_weights_F2$sign == '-', df_weights_F2$value*-1, df_weights_F2$value*1)
df_weights_F2$category <- 'genes'

# Filtering by genes present in 'combined_genes' and by 'val_filter'
df_weights_F2x <- filter(df_weights_F2, feature %in% combined_genes)
df_weights_F2x <- filter(df_weights_F2x, value > val_filter)

# Updating the 'category' column values according to each pathway
df_weights_F2x <- df_weights_F2x %>%
  mutate(
    category = case_when(
      feature %in% g_was ~ 'AD GWAS',
      feature %in% genes_in_more_than_one ~ 'Multiple pathways',
      feature %in% immune ~ 'Immune response',
      feature %in% vasc ~ 'Circulatory system development',
      TRUE ~ 'genes'
    )
  )

# Filtering by genes not present in 'combined_genes' and by 'val_filter'
df_weights_F2 <- filter(df_weights_F2, !(feature %in% combined_genes) | value <= val_filter)

#' Recombining 'df_weights_F2x' and 'df_weights_F2' to restore the original data frame structure, 
#' but now with different values on the column 'category'
df_weights_F2 <- bind_rows(df_weights_F2, df_weights_F2x)

# Rearranging order
df_weights_F2$category <- factor(df_weights_F2$category, levels = c(
  'AD GWAS',
  'Multiple pathways',
  'Immune response',
...
# Display plot
jitter_plot_F2 <- jitter_ploting_weights(df_weights_F2,filtered_val = val_filter, 
                                         categories = c('AD GWAS' = 'black',
                                                        'Circulatory system development' = "blue",
                                                        'Immune response' = "#CC0000FF",
                                                        'Multiple pathways' = 'darkgreen'
                                         ))

ggsave("C:/Users/User/Documents/Drive_PC/Lucas/GitHub/ROSMAP-MOFA/MOFA_real/Figuras_MOFA/pre_inkscape/weight/jitter/new/new_jitter_F2_v6.pdf",
       plot = jitter_plot_F2 , width = 500, height = 255, units = 'mm')