...
all_weights=get_weights(mofa,factors = 26,scale = TRUE, as.data.frame = TRUE)
...
##########################################################
# DISPLAY JITTER PLOT WITH FEATURE WEIGHTS FOR FACTOR 26 #
##########################################################
library(readxl)
library(patchwork)
library(ggplot2)
...
df_weights_F26 <- cleared_df

# A custom vector of genes related to AD
load("C:/Users/User/Documents/Drive_PC/Lucas/GitHub/ROSMAP-MOFA/MOFA_real/Raw_data/weights/F26_gene_vec")
...
repeated_genes <- c("ENG","FOS","SASH1","TNFRSF1B")

# Threshold for filtering feature weights by absolute value
val_filter = 0.5
df_filtered <- filter(df_weights_F26, value > val_filter)

##################################################### Chosen pathways #############################################################################

# Retrieving the genes from the pathways lists

vascular = ALL_DATA_BASE[grep("GOBP_VASCULATURE_DEVELOPMENT", names(ALL_DATA_BASE))]
vascular = unlist(vascular)
vascular = intersect(df_filtered$feature,vascular)

lipid=ALL_DATA_BASE[grep("GOBP_RESPONSE_TO_LIPID", names(ALL_DATA_BASE))]
lipid=unlist(lipid)
lipid=intersect(df_filtered$feature,lipid)
...
combined_genes  = c(vascular,synapse,lipid, g_was)
...
df_weights_F26$real_values <- ifelse(df_weights_F26$sign == '-', df_weights_F26$value*-1, df_weights_F26$value*1)
df_weights_F26$category <- 'genes'

# Filtering by genes present in 'combined_genes' and by 'val_filter'
df_weights_F26x <- filter(df_weights_F26, feature %in% combined_genes)
df_weights_F26x <- filter(df_weights_F26x, value > val_filter)

# Updating the 'category' column values according to each pathway
df_weights_F26x <- df_weights_F26x %>%
  mutate(
    category = case_when(
      feature %in% g_was ~ 'AD GWAS',
      feature %in% genes_in_more_than_one ~ 'Multiple pathways',
      feature %in% vascular ~ 'Vasculature development',
      feature %in% lipid ~ 'Lipid response',
      feature %in% synapse ~ 'Synapse',
      TRUE ~ 'genes'
    )
  )
...
df_weights_F26 <- filter(df_weights_F26, !(feature %in% combined_genes) | value <= val_filter)

#' Recombining 'df_weights_F26x' and 'df_weights_F26' to restore the original data frame structure, 
#' but now with different values on the column 'category'
df_weights_F26 <- bind_rows(df_weights_F26, df_weights_F26x)

# Rearranging order
df_weights_F26$category <- factor(df_weights_F26$category, levels = c(
  'AD GWAS',
  'Multiple pathways',
  'Lipid response',
  'Synapse',
...
# Display plot
jitter_plot_F26 <- jitter_ploting_weights(df_weights_F26,filtered_val = val_filter, 
                                         categories = c(
                                           'AD GWAS' = 'black',
                                           'Vasculature development' = 'darkblue',
                                           'Lipid response' = "#CC0000FF",
                                           'Synapse' = "#E18727FF",
                                           'Multiple pathways' = "darkgreen"
                                         ))

ggsave("C:/Users/User/Documents/Drive_PC/Lucas/GitHub/ROSMAP-MOFA/MOFA_real/Figuras_MOFA/pre_inkscape/weight/jitter/new/new_jitter_F26_v6.pdf",
       plot = jitter_plot_F26 , width = 500, height = 255, units = 'mm')
