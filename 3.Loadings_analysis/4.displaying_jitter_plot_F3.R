...
all_weights=get_weights(mofa,factors = 3,scale = TRUE, as.data.frame = TRUE)
...
#########################################################
# DISPLAY JITTER PLOT WITH FEATURE WEIGHTS FOR FACTOR 3 #
#########################################################
library(MOFA2)
library(readxl)
library(dplyr)
...
jitter_ploting_weights <- function(df,categories,filtered_val){
  ...
  DLPFC_plot <- DLPFC_plot + guides(color = guide_legend(override.aes = list(size = 7)))
...
df_weights_F3 <- cleared_df

# A custom vector of genes related to AD
load("C:/Users/User/Documents/Drive_PC/Lucas/GitHub/ROSMAP-MOFA/MOFA_real/Raw_data/weights/F3_gene_vec")
...
df_weights_F3$real_values <- ifelse(df_weights_F3$sign == '-', df_weights_F3$value*-1, df_weights_F3$value*1)
df_weights_F3$category <- 'genes'

# Filtering by genes present in 'combined_genes' and by 'val_filter'
df_weights_F3x <- filter(df_weights_F3, feature %in% combined_genes)
df_weights_F3x <- filter(df_weights_F3x, value > val_filter)

# Updating the 'category' column values according to each pathway
df_weights_F3x <- df_weights_F3x %>%
  mutate(
    category = case_when(
      feature %in% g_was ~ 'AD GWAS',
      feature %in% genes_in_more_than_one ~ 'Multiple pathways',
      feature %in% cytokine ~ 'Cytokine signaling\nin immune system',
      feature %in% synapse ~ 'Prot-M5 (post synaptic density)',
      TRUE ~ 'genes'
    )
  )

# Filtering by genes not present in 'combined_genes' and by 'val_filter'
df_weights_F3 <- filter(df_weights_F3, !(feature %in% combined_genes) | value <= val_filter)

#' Recombining 'df_weights_F3x' and 'df_weights_F3' to restore the original data frame structure, 
#' but now with different values on the column 'category'
df_weights_F3 <- bind_rows(df_weights_F3, df_weights_F3x)

# Rearranging order
df_weights_F3$category <- factor(df_weights_F3$category, levels = c(
  'AD GWAS',
  'Multiple pathways',
  'Cytokine signaling\nin immune system',
  'Prot-M5 (post synaptic density)',
  "genes")
)

# Display plot
jitter_plot_F3 <- jitter_ploting_weights(df_weights_F3,filtered_val = val_filter, 
                                         categories = c('AD GWAS' = 'black',
                                                        'Cytokine signaling\nin immune system' = "magenta",
                                                        'Prot-M5 (post synaptic density)' = "#631879FF",
                                                        'Multiple pathways' = 'darkgreen'
                                         ))

ggsave("C:/Users/User/Documents/Drive_PC/Lucas/GitHub/ROSMAP-MOFA/MOFA_real/Figuras_MOFA/pre_inkscape/weight/jitter/new/new_jitter_F3_v6.pdf",
       plot = jitter_plot_F3 , width = 500, height = 255, units = 'mm')