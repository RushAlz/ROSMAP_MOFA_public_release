...
all_weights=get_weights(mofa,factors = 4,scale = TRUE, as.data.frame = TRUE)
...
#########################################################
# DISPLAY JITTER PLOT WITH FEATURE WEIGHTS FOR FACTOR 4 #
#########################################################
library(readxl)
library(patchwork)
library(ggplot2)
...
df_weights_F4 <- cleared_df

# A custom vector of genes related to AD
load("C:/Users/User/Documents/Drive_PC/Lucas/GitHub/ROSMAP-MOFA/MOFA_real/Raw_data/weights/F4_gene_vec")
...
val_filter = 0.5
df_filtered <- filter(df_weights_F4, value > val_filter)

##################################################### Chosen pathways ##############################################################################

# Retrieving the genes from the pathways lists

NGF <- ALL_DATA_BASE[grep("GOMF_NEUROPEPTIDE_HORMONE_ACTIVITY", names(ALL_DATA_BASE))]
NGF <- unlist(NGF)
NGF <- unique(NGF)
NGF <- intersect(df_filtered$feature,NGF)

cytokine <- ALL_DATA_BASE[grep("CYTOKINE_SIGNALING_IN_IMMUNE_SYSTEM", names(ALL_DATA_BASE))]
cytokine <- unlist(cytokine)
cytokine <- unique(cytokine)
cytokine <- intersect(df_filtered$feature,cytokine)

MAPK <- MODULES_LIST[["m7_mapk_metabolism"]]
MAPK <- unique(MAPK)
MAPK <- intersect(df_filtered$feature,MAPK)

cytoskeleton <- MODULES_LIST[["m12_cytoskeleton"]]
cytoskeleton <- unique(cytoskeleton)
cytoskeleton <- intersect(df_filtered$feature,cytoskeleton)

# Concatenating genes in a vector
combined_genes  = c(cytokine,NGF,cytoskeleton,MAPK,g_was)
...
df_weights_F4$real_values <- ifelse(df_weights_F4$sign == '-', df_weights_F4$value*-1, df_weights_F4$value*1)
df_weights_F4$category <- 'genes'

# Filtering by genes present in 'combined_genes' and by 'val_filter'
df_weights_F4x <- filter(df_weights_F4, feature %in% combined_genes)
df_weights_F4x <- filter(df_weights_F4x, value > val_filter)

# Updating the 'category' column values according to each pathway
df_weights_F4x <- df_weights_F4x %>%
  mutate(
    category = case_when(
      feature %in% g_was ~ 'AD GWAS',
      feature %in% genes_in_more_than_one ~ 'Multiple pathways',
      feature %in% cytokine ~ 'Cytokine signaling\nin immune system',
      feature %in% NGF ~ 'Neuropeptide hormone activity',
      feature %in% MAPK ~ 'Prot-M7 (MAPK metabolism)',
      feature %in% cytoskeleton ~ 'Prot-m12 (cytoskeleton)',
      TRUE ~ 'genes'
    )
  )

# Filtering by genes not present in 'combined_genes' and by 'val_filter'
df_weights_F4 <- filter(df_weights_F4, !(feature %in% combined_genes) | value <= val_filter)

#' Recombining 'df_weights_F4x' and 'df_weights_F4' to restore the original data frame structure, 
#' but now with different values on the column 'category'
df_weights_F4 <- bind_rows(df_weights_F4, df_weights_F4x)

# Rearranging order
df_weights_F4$category <- factor(df_weights_F4$category, levels = c(
  'AD GWAS',
  'Multiple pathways',
  'Prot-m12 (cytoskeleton)',
  'Cytokine signaling\nin immune system',
  'Neuropeptide hormone activity',
  'Prot-M7 (MAPK metabolism)',
  "genes")
)

# Display plot
jitter_plot_F4 <- jitter_ploting_weights(df_weights_F4,filtered_val = val_filter, 
                                         categories = c('AD GWAS' = 'black',
                                                        'Neuropeptide hormone activity' = "#E18727FF",
                                                        'Prot-m12 (cytoskeleton)' = "#CC0000FF",
                                                        'Cytokine signaling\nin immune system' = "magenta",
                                                        'Prot-M7 (MAPK metabolism)' = "blue",
                                                        'Multiple pathways' = 'darkgreen'
                                         ))

ggsave("C:/Users/User/Documents/Drive_PC/Lucas/GitHub/ROSMAP-MOFA/MOFA_real/Figuras_MOFA/pre_inkscape/weight/jitter/new/new_jitter_F4_v6.pdf",
       plot = jitter_plot_F4 , width = 500, height = 255, units = 'mm')
